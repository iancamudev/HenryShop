...
export function CartItem({id, quantity, color, variante}: CartItemProps) {
    ...
    return (<div className="flex flex-col mx-4 mb-3 p-2 items-center  bg-yellow border rounded-lg shadow-md md:flex-row md:max-w-xl">
            <img src={prod?.image}
            ...
            <div className="flex p-2 w-full justify-between" >
                 <div className="w-full"><div className="text-xl font-bold ">
                    ...
                    <div className="flex mt-1 p-1 px-4 ml-16 justify-center w-44 border border-gray-600 rounded-xl space-x-10" >
                <div className="d-flex  justify-center" style={{gap: ".5rem"}}>
                <Button className="w-9 py-1 px-3 rounded-xl  bg-gray-500 text-white text-xl"  onClick={()=> decreaseCartQuantity(prod?.id as string, color, variante)}>-</Button>
                    </div>
                    <div  className="fs-3 pt-0.5 font-semibold text-2xl">{quantity}</div> 
                    <div> 
                <Button className="w-9 py-1 px-3 rounded-xl bg-gray-500 text-white text-xl"  onClick={() => increaseCartQuantity(prod?.id as string, color, variante)}>+</Button>
                </div>
                </div></div>
