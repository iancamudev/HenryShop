...
const ProductsPanel = () => {
  ...
  const handleActivate = async (
    ...
    event.preventDefault();
    await axios.put(`${URL_BACK_DEV}/products/activate/${event.currentTarget.value}`);
    dispatch(getAllProductsAdmin(currentPage, filters));
  };
  return (
      ...
      <div className=" mt-8 mb-8 flex flex-col bg-slate-900 justify-center xl:w-10/12 shadow">
        ...
        <table className="shadow-2xl">
          ...
          <tr className="border border-black bg-slate-900	text-white rounded-xl ">
            ...
            <th className="border border-black font-normal p-2 xl:w-2/4">Nombre</th>
            <th className="border border-black font-normal xl:w-3/12 p-2 pl-4 pr-4">Precio</th>
            <th className="border border-black font-normal xl:w-3/12 p-2 pl-4 pr-4">Actions</th>
          </tr>
          {Products &&
            Products.map((producto) => (
              <tr className="border border-slate-300">
                ...
                >
                  {<p className="text-xs font-bold items-center justify-center p-3 m-3 ">${producto.price[producto.price.length -1]}</p>}
                  </td>
                  <td className={
                    id % 2 !== 0
                      ? "bg-gray-300"
